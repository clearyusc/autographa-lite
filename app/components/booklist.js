...
class BookList extends React.Component {
	...
	goToTab(key) {
		...
		global.bookName = bookCodeList[parseInt(bookno, 10) - 1]
...
var BookGroup = function(props) {
	const BooksGroup = props.result.map((item,index) =>{
		...
		return <li key={index}><a href="#" onClick={_handleClick } value={item} className={(index+1 == props.currentBook) ? 'link-active': ""}>{item}
		</a></li>
	})
	return (
		<div>{BooksGroup}</div>
